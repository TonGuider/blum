...
if __name__ == "__main__":
    logger = Logger("[https://t.me/Ton_Guider]")
    logger.log("Автокликер Blum")
    ...
    while answer is None:
        points_key = logger.input(
            "Сколько хотите очков? | 1 -> 90-110 | 2 -> 140-160 | 3 -> 170-180 | 4 -> MAX: ")
    ...
    while answer is None:
        points_key = logger.input("Запустить автозапуск игр? | 1 - да / 0 - нет: ")
        answer = answs.get(points_key, None)
        if answer is None:
            logger.log("Неверное значение")
    is_continue = answer

    logger.log('После запуска игры нажмите ё (`)')
